...
function parse_indicator(json::Array{Any,1})::DataFrame
    ...
    DataFrame(
        ...
        source_database = source_database_val,
...
function download_indicators(; verbose::Bool = false)::DataFrame
    dat = download_parse_json(
        "https://api.worldbank.org/v2/indicators?per_page=25000&format=json",
        verbose = verbose,
    )
...
function download_countries(; verbose::Bool = false)::DataFrame
    dat = download_parse_json(
        "https://api.worldbank.org/v2/countries/all?per_page=25000&format=json",
        verbose = verbose,
    )
...
df_match(df::AbstractDataFrame, entry::String, regex::Regex)::DataFrame =
    df[occursin.(Ref(regex), df[!, make_symbol(entry)]), :]
...
function search_wdi(data::String, entry::String, regx::Regex)::DataFrame
    ...
    elseif data == "indicators"
        ...
        error(
            ...
            data,
            ". supported are: \"countries\" or \"indicators\"",
        )
    end
end
...
function clean_append!(
    vals::Union{Array{String,1},Array{String,1}},
    val::Union{String,String,Nothing},
)
    append!(vals, [clean_entry(val)])
end

function parse_wdi(
    indicators::Array{String,1},
    jsondata::Array{Any,1},
    startyear::Integer,
    endyear::Integer,
    returnlong::Bool,
)::DataFrame
...
function get_url(
    indicators::Union{String,Array{String,1}},
    countries::Union{String,Array{String,1}},
    startyear::Integer,
    endyear::Integer,
    sourceid::Integer;
    verbose::Bool = false,
